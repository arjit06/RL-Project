...
prev_score=0
flappy=False
def get_reward(reward,done,info): 
    if 'score' not in info: # not a flappy bird env
        return reward 
...
class ClipRewardEnv(gym.RewardWrapper):
    ...
    def step(self, action):
        ...
        reward=get_reward(reward,done,info)
    ...
    def reward(self, reward):
        global flappy
        if flappy==False: return (reward > 0) - (reward < 0)
        else: return reward
