...
class SimPLe:
    ...
    def collect_interactions(self):
        ...
        for _ in trange(6400, desc='Collecting interactions'):
            ...
            obs, re, done, _ = self.real_env.step(action)
            if done[0]:
                obs = self.real_env.reset()
    ...
    def train_agent_sim_env(self, epoch, eval_period=100):
        ...
        if self.config.save_models:
            self.agent.save(os.path.join('models', f'ppo_{self.config.env_name}.pt'))
    ...
    def load_models(self):
        self.model.load_state_dict(torch.load(f'./models/model_{self.config.env_name}.pt',map_location=torch.device('cpu')))
        self.agent = PPO(self.simulated_env, config.device, num_steps=self.config.rollout_length, num_mini_batch=5)
        self.agent.load(f'./models/ppo_{self.config.env_name}.pt')
...
if __name__ == '__main__':
    ...
    if config.load_models:
        simple.load_models()
        eval_metrics = simple.evaluate_agent()
        print(eval_metrics)
    else:
        simple.train()
